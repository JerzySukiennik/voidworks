...
export async function createWorld(view, orbit) {
  ...
  function pieceFor(inDir, outDir, base) {
    if (outDir === inDir || outDir < 0) return base;
    if (outDir === turnLeft(inDir)) return 'belt_turn_l';
    if (outDir === turnRight(inDir)) return 'belt_turn_r';
    return base;
  }
  ...
  function planRun(cells, straightId) {
    ...
    for (let i = 0; i < cells.length; i += 1) {
      ...
      plan.push({ id: pieceFor(inDir, outDir, base), cx: cur[0], cz: cur[1], rot: inDir });
    }
    return plan;
  }
  ...
  function layPath(cells, specials, opts) {
    ...
    while (i < cells.length) {
      ...
      const outDir = next ? dirBetween(cur[0], cur[1], next[0], next[1]) : inDir;
      place(pieceFor(inDir, outDir, 'belt'), cur[0], cur[1], inDir, opts);
      i += 1;
    }
  }
  ...
  // --- starter factory ---------------------------------------------------------
  // Square in plan and two storeys tall, because this is the composition every screenshot is
  // framed on: four feeder rows east into a collector spine, the spine south into a row of gates
  // running back west, then a split into two returns — one climbing to an upper deck that crosses
  // over all four feeders, one hugging the west edge — both landing on the same sell pad.
  // Read it as a spiral: drop, gather, upgrade, cross, sell.

  function buildStarter() {
    const free = { free: true };
    const FEED_Z = [-4, -3, -2, -1, 0];

    // five feeders packed shoulder to shoulder, cheap ones and better ones, so the belts run mixed
    const sources = ['dropper_scrap', 'dropper_scrap', 'dropper_ore', 'dropper_ore', 'dropper_scrap'];
    FEED_Z.forEach((z, i) => {
      place(sources[i], -11, z, 0, free);
      for (let x = -10; x <= -2; x += 1) place('belt', x, z, 0, free);
    });

    // collector spine running south, taking each feeder in through a merger, then splitting the
    // gathered flow across two gate rows
    place('belt_turn_r', -1, -4, 0, free);
    for (let z = -3; z <= 1; z += 1) {
      place(FEED_Z.includes(z) ? 'belt_merge' : 'belt', -1, z, 3, free);
    }
    place('belt_split', -1, 2, 3, free);
    place('belt_turn_r', -1, 3, 3, free);

    // two gate rows running back west, adders before multipliers — which is also the order that
    // pays best, and staggered between rows so the colour ramp reads as a block
    const rowA = new Map([[-3, 'add_tack'], [-5, 'add_stamp'], [-7, 'mul_125'], [-9, 'mul_15']]);
    const rowB = new Map([[-4, 'add_tack'], [-6, 'add_stamp'], [-8, 'mul_125'], [-10, 'mul_15']]);
    for (let x = -2; x >= -10; x -= 1) place(rowA.get(x) || 'belt', x, 2, 2, free);
    place('belt_turn_l', -11, 2, 2, free);
    for (let x = -2; x >= -10; x -= 1) place(rowB.get(x) || 'belt', x, 3, 2, free);
    place('belt_merge', -11, 3, 2, free);
    place('belt', -12, 3, 2, free);
    place('belt_turn_r', -13, 3, 2, free);

    // upper deck: climbs at the west corner, runs back east over the yard, then north across
    // every one of the five feeders below
    place('belt_ramp_up', -13, 2, 1, free);
    place('belt_sky_turn_r', -13, 1, 1, free);
    for (let x = -12; x <= -6; x += 1) place('belt_elev', x, 1, 0, free);
    place('belt_sky_turn_l', -5, 1, 0, free);
    for (let z = 0; z >= -4; z -= 1) place('belt_elev', -5, z, 1, free);
    place('belt_ramp_down', -5, -5, 1, free);

    // back down to ground level and west along the top edge, so the pad anchors the far corner
    place('belt_turn_l', -5, -6, 1, free);
    for (let x = -6; x >= -11; x -= 1) place('belt', x, -6, 2, free);
    place('sellpad', -13, -7, 0, free);

    flow.relink();
    flow.spawnBurst(Math.floor(economy.capacity * 0.85));
  }
  ...
  function buildBench(target) {
    ...
    place('belt_turn_r', -16, 4, 0, free);
    place('belt_turn_l', -16, 5, 3, free);
