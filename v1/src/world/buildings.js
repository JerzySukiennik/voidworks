...
export const BUILDINGS = [
  ...
  def({
    id: 'belt_turn_l', name: 'Curve Left', family: 'belt', cost: 30,
    desc: 'Turns the run 90 degrees to the left.',
    lanes: [turnLeftLane()], parts: cornerParts(-1),
  }),
  def({
    id: 'belt_turn_r', name: 'Curve Right', family: 'belt', cost: 30,
    desc: 'Turns the run 90 degrees to the right.',
    lanes: [turnRightLane()], parts: cornerParts(1),
  }),
  def({
  ...
  def({
    id: 'belt_sky_turn_l', name: 'Sky Curve Left', family: 'belt', cost: 55, unlock: 1,
    levels: [1], desc: 'Turns the upper deck 90 degrees to the left.',
    lanes: [turnLeftLane(RISE)],
    parts: [...cornerParts(-1, RISE), part('box', 'steelDark', 0, BY + RISE - 0.32, 0, 0.34, 0.34, 0.34)],
  }),
  def({
    id: 'belt_sky_turn_r', name: 'Sky Curve Right', family: 'belt', cost: 55, unlock: 1,
    levels: [1], desc: 'Turns the upper deck 90 degrees to the right.',
    lanes: [turnRightLane(RISE)],
    parts: [...cornerParts(1, RISE), part('box', 'steelDark', 0, BY + RISE - 0.32, 0, 0.34, 0.34, 0.34)],
  }),
  ...
  def({
    ...
    cells: CELLS_2x2, rotatable: false,
    desc: 'Anything that touches it turns into money and frees the slot it was using.',
    lanes: sinkLanes(CELLS_2x2, 0.5, 0.5, 0),
    parts: [
      part('box', 'accentDeep', 0.5, BY - 0.17, 0.5, 2.0, 0.24, 2.0),
      part('box', 'accent', 0.5, BY - 0.02, 0.5, 1.78, 0.09, 1.78),
      part('box', 'accentDeep', 0.5, BY + 0.045, 0.5, 0.9, 0.06, 0.9),
      part('cyl', 'accent', 0.5, BY + 0.15, 0.5, 0.34, 0.28, 0.34),
    ],
  }),
  def({
...
// --- authored meshes ----------------------------------------------------------
// One glb per family; `modelRot` is the extra quarter turn a shared mesh needs. belt-curve joins the
// -X face to the -Z face, which IS a left turn at rot 0, so a right turn is the same mesh turned once.
const MODEL_FOR = {
  belt: 'belt-straight',
  belt_fast: 'belt-straight',
  belt_turn_l: 'belt-curve',
  belt_turn_r: 'belt-curve',
  belt_sky_turn_l: 'belt-curve',
  belt_sky_turn_r: 'belt-curve',
...
for (const d of BUILDINGS) {
  ...
  d.modelRot = d.id === 'belt_turn_r' || d.id === 'belt_sky_turn_r' ? 1 : 0;
