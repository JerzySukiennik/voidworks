...
const BELT_EFFECT = {
  belt_turn_l: 'left', belt_turn_r: 'right', belt_merge: '3→1', belt_split: '1→3',
...
export function createBuildbar(world) {
  ...
  function onKey(e) {
    ...
    if (e.key === 'q' || e.key === 'Q') { cycleGroup(-1); e.preventDefault(); return; }
    if (e.key === 'e' || e.key === 'E') { cycleGroup(1); e.preventDefault(); return; }
    if (e.key >= '1' && e.key <= '9') {
